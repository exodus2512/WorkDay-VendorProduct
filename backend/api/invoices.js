...
/**
 * --------------------------------------------------------------------------------
 * INVOICES API HANDLER (/api/invoices)
 * --------------------------------------------------------------------------------
 * Core Logic & Workflow:
 *  - Handles client-facing milestone invoice creation, compliance validation, status state transitions,
 *    and vector PDF generation.
 *  - Enforces the 7-Point Billing Compliance Engine (`validateInvoice`) before allowing invoice generation.
 *  - Uses asynchronous queue processing (`addInvoiceGenerationJob` via BullMQ) to offload PDF vector rendering
 *    and background email generation without blocking web API requests.
 *  - GATED PAYROLL RELEASE: When an invoice status transitions to `PAID`, contractor payrolls for
 *    the corresponding project milestones are automatically unlocked and marked `PROCESSED`.
 *
 * Supported Operations:
 *  - GET /api/invoices
 *      Lists invoices enriched with line items. Filterable by `client_name`, `client_id`, `project_id`.
 *  - GET /api/invoices/:id
 *      Returns full invoice details including `invoice_items` line item breakdown.
 *  - GET /api/invoices/:id/pdf (or /download)
 *      Generates and returns vector PDF invoice file for client download.
 *  - POST /api/invoices/validate
 *      Runs the 7-Point Compliance Audit check on demand and returns detailed pass/fail rule results.
 *  - POST /api/invoices (or /api/invoices/generate-async)
 *      Runs pre-submission audit check and enqueues an asynchronous generation job via BullMQ.
 *  - GET /api/invoices/job-status/:jobId
 *      Polls BullMQ job queue for progress (20% -> 100%) and returns generated invoice ID once complete.
 *  - PUT /api/invoices/:id
 *      Transitions invoice status (`DRAFT` -> `SENT` -> `APPROVED` -> `PAID`). When `PAID`, releases contractor payroll.
 * --------------------------------------------------------------------------------
 */
