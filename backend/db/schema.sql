...
CREATE TABLE users (
  ...
  -- Multi-currency & tax region fields
  payout_currency VARCHAR(3) NOT NULL DEFAULT 'USD',
  tax_region VARCHAR(50) DEFAULT 'US-DEFAULT',  -- e.g. 'IN-GST', 'UK-VAT', 'EU-VAT', 'US-TX'
  tax_exempt BOOLEAN NOT NULL DEFAULT FALSE,     -- TRUE = B2B contractor, self-manages tax
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE projects (
  ...
  status VARCHAR(50) DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'ACTIVE', 'COMPLETED', 'REJECTED')),
  project_manager_id INT REFERENCES users(id) ON DELETE SET NULL,
  billing_currency VARCHAR(3) NOT NULL DEFAULT 'USD',  -- Base currency for client invoices
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE assignments (
  ...
  weekly_hour_limit INT NOT NULL DEFAULT 40,
  status VARCHAR(50) DEFAULT 'ACTIVE' CHECK (status IN ('ACTIVE', 'EXPIRING_SOON', 'COMPLETED', 'INACTIVE')),
  fixed_exchange_rate NUMERIC(12, 6) DEFAULT NULL, -- NULL = use live rate or SET = locked rate for payroll
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE invoices (
  ...
  tax NUMERIC(12, 2) NOT NULL DEFAULT 0.00,
  total NUMERIC(12, 2) NOT NULL DEFAULT 0.00,
  status VARCHAR(50) DEFAULT 'DRAFT' CHECK (status IN ('DRAFT', 'SUBMITTED', 'SENT', 'APPROVED', 'REJECTED', 'PAID')),
  billing_currency VARCHAR(3) NOT NULL DEFAULT 'USD',   -- Locked at invoice generation time
  payment_currency VARCHAR(3),                          -- Selected by client at payment time
  payment_exchange_rate NUMERIC(12, 6),                 -- Exchange rate applied at payment time
  tax_rate_applied NUMERIC(5,4) NOT NULL DEFAULT 0.18,  -- Locked rate used (e.g. 0.18)
  idempotency_key VARCHAR(255) UNIQUE,
  pdf_data TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE contractor_payrolls (
  ...
  assignment_id INT REFERENCES assignments(id) ON DELETE CASCADE,
  total_hours NUMERIC(10, 2) NOT NULL DEFAULT 0.00,
  billing_rate NUMERIC(10, 2) NOT NULL DEFAULT 0.00,       -- In project base currency (e.g. USD)
  gross_pay NUMERIC(12, 2) NOT NULL DEFAULT 0.00,          -- In project base currency (legacy/USD)
  -- Multi-currency payroll fields
  payout_currency VARCHAR(3) NOT NULL DEFAULT 'USD',
  exchange_rate_used NUMERIC(12,6) NOT NULL DEFAULT 1.0,   -- Rate snapshot at generation time
  gross_pay_local NUMERIC(14,2) NOT NULL DEFAULT 0.00,     -- gross_pay * exchange_rate (in payout_currency)
  tax_rate_applied NUMERIC(5,4) NOT NULL DEFAULT 0.0,      -- e.g. 0.18 for IN-GST, 0 for tax_exempt
  tax_amount_local NUMERIC(14,2) NOT NULL DEFAULT 0.00,    -- gross_pay_local * tax_rate_applied
  net_pay_local NUMERIC(14,2) NOT NULL DEFAULT 0.00,       -- gross_pay_local - tax_amount_local
  status VARCHAR(50) DEFAULT 'PROCESSED' CHECK (status IN ('PENDING', 'PROCESSED', 'PAID')),
  idempotency_key VARCHAR(255) UNIQUE,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
