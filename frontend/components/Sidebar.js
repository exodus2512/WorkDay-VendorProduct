...
export default function Sidebar({ currentRole, activeSection, onSelectSection }) {
  ...
  return (
    <aside className="w-64 bg-slate-950 text-slate-300 flex flex-col flex-shrink-0 min-h-screen border-r border-slate-800/80">
      {/* Brand Header */}
      <div className="p-6 border-b border-slate-800/80 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-400 font-bold tracking-wider">VENDOR OS</p>
        </div>
      </div>

      {/* Role Pill */}
      <div className="px-6 py-4 bg-slate-900/60 border-b border-slate-800/80">
        <span className="text-[10px] uppercase font-extrabold tracking-wider text-slate-500 block mb-1">Active Portal</span>
        ...
        {navItems.map(item => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-bold text-sm transition-all duration-150 active:scale-[0.98] ${
                isActive
      ...
      <div className="p-4 border-t border-slate-800/80 text-xs text-slate-500 text-center">
        <p className="font-bold text-slate-400">Contingent Tracker MVP</p>
        <p className="mt-0.5 text-[11px] font-medium text-slate-500">Multi-Tenant Vendor OS</p>
      </div>
    </aside>
  );
}
