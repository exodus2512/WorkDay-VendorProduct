...
export default function ClientPortal({ clientUser, onRefresh }) {
  ...
  const loadMyProjectsAndPMs = async () => {
    ...
    try {
      const [projRes, pmRes] = await Promise.all([
        fetch(`/api/projects?client_user_id=${clientUser.id}`),
        fetch(`/api/employees?role=PROJECT_MANAGER&vendor_id=${clientUser.vendor_id || 1}`)
      ]);
      const projData = await projRes.json();
      const pmData = await pmRes.json();

      // Robust matching: by client_id, client_name, client_user_id, or demo client link
      const mine = Array.isArray(projData) ? projData.filter(p => {
        if (clientUser.client_id && p.client_id && Number(clientUser.client_id) === Number(p.client_id)) return true;
        if (p.client_name && clientUser.name) {
          const pName = p.client_name.toLowerCase().trim();
          const uName = clientUser.name.toLowerCase().trim();
          if (pName === uName || pName.includes(uName) || uName.includes(pName)) return true;
        }
        // Link demo client user 12 (client@client.com / Client Corp / Apex Financial) to project 1 (Apex Financial Services)
        if (
          clientUser.email === 'client@client.com' ||
          clientUser.id === 12 ||
          clientUser.name === 'Client Corp' ||
          clientUser.name === 'Apex Financial'
        ) {
          if (
            p.client_name === 'Apex Financial Services' ||
            p.client_name === 'Apex Financial' ||
            p.client_name === 'Client Corp' ||
            Number(p.id) === 1
          ) {
            return true;
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const projRes = await fetch('/api/projects', {
        ...
        body: JSON.stringify({
          name: form.name,
          client_name: clientUser?.name || 'Apex Financial',
          description: form.description,
          budget: form.budget,
          start_date: form.start_date,
          end_date: form.end_date,
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Client Delivery & Invoicing Portal</h1>
              <p className="text-sm text-slate-500 mt-0.5">
                Welcome, <span className="font-semibold text-blue-600">{clientUser.name}</span> — Track project delivery milestones, review invoices, and download settlement PDFs.
              </p>
            </div>
          </div>

          {/* Tab Navigation */}
          <div className="flex gap-2 mt-6 border-b border-slate-200 pb-0 overflow-x-auto">
            <button
              onClick={() => setActiveTab('projects')}
              className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-t-lg transition-all ${activeTab === 'projects' ? 'bg-blue-600 text-white shadow' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-t-lg transition-all ${activeTab === 'invoices' ? 'bg-blue-600 text-white shadow' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50'}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-t-lg transition-all ${activeTab === 'pms' ? 'bg-blue-600 text-white shadow' : 'text-slate-600 hover:text-blue-600 hover:bg-blue-50'}`}
            >
        ...
        {activeTab === 'pms' && (
          <ProjectManagersTab clientUser={clientUser} />
        )}
...
function ProjectCard({ project, statusColor, onViewInvoices }) {
  const [milestones, setMilestones] = useState([]);
  const [expanded, setExpanded] = useState(false);
  ...
  const approvedMs = milestones.filter(m => m.status === 'APPROVED' || m.status === 'COMPLETED').length;
  const allMilestonesApproved = milestones.length > 0 && approvedMs === milestones.length;
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden transition-all hover:shadow-md">
      <div className="p-6">
            ...
            <div className="flex items-center gap-3 mb-1">
              ...
              {allMilestonesApproved && (
                <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5" /> All Milestones Approved
                </span>
              )}
            </div>
            <p className="text-sm text-slate-500 line-clamp-2">{project.description}</p>
        ...
        <div className="mt-4 flex gap-2">
          <button
            onClick={() => setExpanded(!expanded)}
            className="flex-1 flex items-center justify-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-800 py-2 rounded-lg hover:bg-blue-50 transition-colors border border-blue-100"
          >
            <Eye className="w-4 h-4" />
            {expanded ? 'Hide Milestones' : 'View Milestones'}
          </button>
          {onViewInvoices && (
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-4 py-2 text-sm font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors"
            >
